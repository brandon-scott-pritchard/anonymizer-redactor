...
def _set_group_text(group: list[etree._Element], hits: list[Hit], redact: bool) -> set:
    ...
    for name, data in list(payload.items()):
        ...
        if TEXT_PART_RE.match(name):
            try:
                ...
                result.warnings.append(f"could not parse {name}; left unchanged")
                continue
            ...
            for group in _paragraph_texts(root):
                ...
                touched = _set_group_text(group, hits, redact)
                if redact:
                    _black_bar(touched)
